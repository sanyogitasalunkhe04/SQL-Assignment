...
--3.
select *, CONCAT(first_name,',',job_id) as employeetitle from employees

--4.
	
select hire_date, first_name, department_id, job_id from employees where job_id ='13' or job_id ='17'or job_id ='18'
...
--9
select First_name, salary from employees order by hire_date desc

--10
select First_name, department_id from employees order by hire_date desc where commision != 'null'

--11
select employees.last_name, jobs.job_title from employees join jobs on employees.job_id = jobs.job_id where jobs.job_id = '4'

--12
select last_name, jobs.job_title, salary from employees join jobs on employees.job_id = jobs.job_id where jobs.job_title = 'sales representative' OR jobs.job_title = 'purchasing clerk' and salary!=2500 and salary!=3500 and salary!=5000 
...
--2.
select concat(first_name,'-',last_name) as fullname, datename(MONTH, "hire_date") as monthofjoining from employees
...
--4.
select left(employees.employee_id,2)+'00'+SUBSTRING(cast(employees.employee_id as varchar),4,LEN(employees.employee_id))+'-E' as OUTPUT, employees.department_id,employees.salary,upper(jobs.job_title) as MANAGER_NAME from employees inner join jobs on employees.job_id= jobs.job_id;

--5
select upper(left(last_name,1))+SUBSTRING(last_name,2,LEN(last_name)) as Last_Name, len(last_name) as Length from employees where last_name like 'J%' OR last_name like 'A%' OR last_name like 'M%' group by last_name

--6   
select last_name,LEFT('$$$$$$$$$$$$' +cast(salary as varchar) ,15 ) as SALARY from employees;

--7
with result as(select last_name, REVERSE(last_name)as Reverse from employees)select last_name,reverse,case when reverse = last_name then 'yes' else 'no' end as palindrome from result
   
Aggregators. a

--1.
select max(Salary) as max_sal, min(salary) as mim_sal, Avg(Salary) as Avg_sal from employees

--2///////////////////////
select department_id, sum(salary) from department group by department_id

--3
select department_id, employee.department count (*) from department group by employee.department;

--4///////////////////////
select department_id, sum(salary) from department group by department_id

--5
select first_name || ' ' || last_name as full_name, salary from employees where commision is null

--6
select first_name || ' ' || last_name as full_name, salary from employees where commision is null

--7
select first_name, salary, case when (commision * 2)>0 then (select (commision*2) from employees) else 'no commision' end from employees

--8
select first_name, count(*) from employees from by first_name having count(*) >1

--9
select sum(salary),managerID, count(ID), count(ID) from employees group by manager id;

--12
select round(avg(salary),0) 'average' group by department_id from employees
